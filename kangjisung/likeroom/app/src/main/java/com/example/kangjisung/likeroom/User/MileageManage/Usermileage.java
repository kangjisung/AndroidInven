...
///////////////////////////////마일리지 적립전 유저 검색 클래스
public class Usermileage extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        scView.setOnQueryTextListener(new SearchView.OnQueryTextListener() {
            ...
            public boolean onQueryTextSubmit(String s) {
                ...
                //////////////////////////검색하여 데이터 넣기
                String View=s;
                new ClientDataBase("select `회원정보`.`이름`,`회원정보`.`전화번호`,`포인트`.`포인트` from `회원정보` join `포인트` on `회원정보`.`고유회원등록번호`= `포인트`.`고유회원등록번호`  where `전화번호` LIKE \"%"+View+"%\"or `이름` LIKE \"%"+View+"%\";",1,3,getApplicationContext());
                int cnt=0;
                while(true) {
